frappe.pages['mappedbom-comparison'].on_page_load = function(wrapper) {
	var page = frappe.ui.make_app_page({
		parent: wrapper,
		title: 'Mapped BOM Comparison Tool',
		single_column: true
	});
	new erpnext.MappedBOMComparisonTool(page);
}
erpnext.MappedBOMComparisonTool = class MappedBOMComparisonTool {
	constructor(page) {
		this.page = page;
		this.make_form();
	}
	...
	fetch_and_render() {
		...
		frappe.call('instrument.instrument.doctype.mapped_bom.mapped_bom.get_bom_diff', {
			bom1: name1,
			bom2: name2
		}).then(r => {
			let diff = r.message;
			frappe.model.with_doctype('Mapped BOM', () => {
				this.render('Mapped BOM', name1, name2, diff);
			});
		});
	}
