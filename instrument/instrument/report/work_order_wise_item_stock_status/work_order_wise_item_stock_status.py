...
def _execute(filters, additional_table_columns=None, additional_query_columns=None):
	...
	for row in wo_list:
		...
		ohs_for_wip = get_current_stock_for_wip()
		if doc.from_wip_warehouse:
			for item in doc.required_items:
				if item.item_code in ohs_for_wip:
					if item.required_qty <= ohs_for_wip.get(item.item_code):
					...
					elif item.required_qty > ohs_for_wip.get(item.item_code) and ohs.get(item.item_code) > 0: 
						final_item_status.append('Partial Qty Available')
						percent_stock = (ohs_for_wip.get(item.item_code)/item.required_qty*100)
			...
			for item in doc.required_items:
				if item.transferred_qty == item.required_qty:
					remaining_stock = (item.transferred_qty- item.consumed_qty)
					final_item_status.append('Full Qty Available')
					percent_stock = 100
					qty_will_be_produced = item.required_qty
					final_item_percent.append(qty_will_be_produced)
				elif item.transferred_qty < item.required_qty and item.transferred_qty > 0:
					final_item_status.append('Partial Qty Available')
					percent_stock = (item.transferred_qty/item.required_qty*100)
					qty_will_be_produced = (percent_stock/100*item.required_qty)
					final_item_percent.append(qty_will_be_produced)
				else:
...
def get_work_orders(filters, additional_query_columns):
	...
	return frappe.db.sql("""
		...
		from `tabWork Order`
		where status in ('Not Started','In Process') and docstatus = 1 %s order by planned_start_date desc, name desc"""%
